            ...
            <div class="card-body pt-0">
               ...
               <table id="kt_datatable_dom_positioning" class="table table-striped table-row-bordered gy-5 gs-7 border rounded">
                    ...
                    <tbody class="fw-semibold text-gray-600">
                        ...
                        <!--begin::Table row-->
                         ...
                         if($_POST){
                                       ...
                                       extract($_POST);
                                            ...
                                            if($agent_id!=''){    
                                                $query[] =  "agents.agent_id='$agent_id'";
                                            }
                                            if($method!=''){ 
                                            if($method=='PayPal'){
                                                $query[] =  "payments.payment_type IS NULL";
                                            }else{
                                                $query[] =  "payments.payment_type='$method'";
                                            }   
                                                
                                            }
                                            if($payment_status!=''){    
                                                $query[] =  "users.payment_status='$payment_status'";
                                            }
                                            if(count($query)){
                                                $where_imp = implode(" AND ",$query);
                                                $where =    "( $where_imp ) ";
                                            }else{
                                            $where = "";
                                            }
                        ...
                        $query= mysqli_query($con, "select *, payments.id as pid from payments  join users on users.id=payments.user_id left join agents on agents.agent_id=users.agent_id where $where ");
